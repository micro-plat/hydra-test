...
func TestAPINew(t *testing.T) {
	tests := []struct {
		...
		{name: "1. Conf-APINew-默认初始化", want: &api.Server{Status: "start", RTimeout: 30, WTimeout: 30, RHTimeout: 30}},
		{name: "2. Conf-APINew-设置address初始化", address: "8080", want: &api.Server{Address: "8080", Status: "start", RTimeout: 30, WTimeout: 30, RHTimeout: 30}},
		...
		{name: "4. Conf-APINew-设置disable初始化", opts: []api.Option{api.WithDisable()}, want: &api.Server{Status: "stop", RTimeout: 30, WTimeout: 30, RHTimeout: 30}},
		{name: "5. Conf-APINew-设置Enable初始化", opts: []api.Option{api.WithEnable()}, want: &api.Server{Status: "start", RTimeout: 30, WTimeout: 30, RHTimeout: 30}},
	}
