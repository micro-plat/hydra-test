...
func TestQueueRedisNew(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "1. Conf-QueueRedisNew-测试新增-无option", opts: []queueredis.Option{queueredis.WithAddrs("192.168.5.79:6379")},
			want: &queueredis.Redis{Queue: &queue.Queue{Proto: "redis"}, Addrs: []string{"192.168.5.79:6379"}, DbIndex: 0, DialTimeout: 0, ReadTimeout: 0, WriteTimeout: 0, PoolSize: 0}},
		{name: "2. Conf-QueueRedisNew-测试新增-WithDbIndex", opts: []queueredis.Option{queueredis.WithAddrs("192.168.5.79:6379"), queueredis.WithDbIndex(2)},
			want: &queueredis.Redis{Queue: &queue.Queue{Proto: "redis"}, Addrs: []string{"192.168.5.79:6379"}, DbIndex: 2, DialTimeout: 0, ReadTimeout: 0, WriteTimeout: 0, PoolSize: 0}},
		{name: "3. Conf-QueueRedisNew-测试新增-WithTimeout", opts: []queueredis.Option{queueredis.WithAddrs("192.168.5.79:6379"), queueredis.WithDbIndex(2), queueredis.WithTimeout(11, 22, 33)},
			want: &queueredis.Redis{Queue: &queue.Queue{Proto: "redis"}, Addrs: []string{"192.168.5.79:6379"}, DbIndex: 2, DialTimeout: 11, ReadTimeout: 22, WriteTimeout: 33, PoolSize: 0}},
