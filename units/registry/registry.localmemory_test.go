...
func TestWatchChildren(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "3. LMWatch-修改节点数据,有数据变更", isE: true, isD: false, path: "/hyd434ra2", value: "2", nvalue: "234", children: []string{"efg"}},
		{name: "4. LMWatch-修改节点数据,空->有", isE: true, isD: false, path: "/hydr545a2", value: "", nvalue: "234", children: []string{"efg"}},
		{name: "5. LMWatch-修改节点数据,有->空", isE: true, isD: false, path: "/hy565dra2", value: "2", nvalue: "", children: []string{"efg"}},
		{name: "6. LMWatch-修改节点数据，值不变", isE: true, isD: false, path: "/hyd678ra2", value: "234", nvalue: "234", children: []string{"efg"}},
	}

	for _, c := range cases {
		//创建节点
		if c.isE {
			err := lm.CreatePersistentNode(registry.Join(c.path, c.children[0]), c.value)
			assert.Equal(t, nil, err, c.name)
		}
