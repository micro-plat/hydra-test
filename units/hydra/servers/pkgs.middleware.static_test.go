...
func TestStatic(t *testing.T) {
	tests := []struct {
		...
		{name: "2.2 static-配置存在-不启用-配置Static出错", isBool: true, opts: []static.Option{static.WithDisable(), static.WithArchive("pkgs.middleware.middle.test.txt")}, responseStatus: 200, wantStatus: 510, wantContent: "解压失败"},
		...
		{name: "3.2 static-配置存在-启用-配置Static出错", isBool: true, opts: []static.Option{static.WithArchive("pkgs.middleware.middle.test.txt")}, responseStatus: 200, wantStatus: 510, wantContent: "解压失败"},
		...
		{name: "3.4 static-配置存在-启用-请求是静态文件,但不存在", isBool: true, requestPath: "/robots.txt", method: "GET", responseStatus: 200, wantStatus: 404, wantContent: "找不到文件", wantSpecial: "static"},
		{name: "3.5 static-配置存在-启用-请求是静态文件夹", isBool: true, opts: []static.Option{static.WithRoot("./")}, requestPath: "pkgs.middleware.static_test", method: "GET", responseStatus: 200, wantStatus: 404, wantContent: "找不到文件", wantSpecial: "static"},
