...
func init() {
	hydra.Conf.API(":8070").Ras(ras.WithAuths(
		ras.New("/single/hydra/newversion/md5/auth@authserver.sas_debug",
			...
			ras.WithCheckTimestamp(false),                   //设置是否需要时间戳校验（不设默认：true）
...
// apiserver_ras ras签名验证自定义配置测试demo2
...
//1.2 调用验签接口，正常签名延迟请求：http://localhost:8070/hydratest/apiserverras/test  请求正常返回成功
