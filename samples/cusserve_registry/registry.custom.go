...
func NewFileSystem(rootDir string) (*customFS, error) {
...
func (l *customFS) formatPath(path string) string {
...
func (l *customFS) getDataPath(path string) string {
...
func (l *customFS) Exists(path string) (bool, error) {
	_, err := os.Stat(l.formatPath(path))
	return err == nil || os.IsExist(err), nil
}

func (l *customFS) GetValue(path string) (data []byte, version int32, err error) {
...
func (l *customFS) Update(path string, data string) (err error) {
...
func (l *customFS) GetChildren(path string) (paths []string, version int32, err error) {
	rpath := l.formatPath(path)
	fs, err := os.Stat(rpath)
	if os.IsNotExist(err) {
...
func (l *customFS) WatchValue(path string) (data chan registry.ValueWatcher, err error) {
...
func (l *customFS) WatchChildren(path string) (data chan registry.ChildrenWatcher, err error) {
	realPath := l.formatPath(path)
	_, err = os.Stat(realPath)
	if os.IsNotExist(err) {
		err = fmt.Errorf("Watch path:%s 不存在", path)
...
func (l *customFS) Delete(path string) error {
	return os.RemoveAll(l.formatPath(path))
}

func (l *customFS) CreatePersistentNode(path string, data string) (err error) {
...
func (l *customFS) createDirPath(path string) error {
	realPath := l.formatPath(path)
	_, err := os.Stat(realPath)
	if os.IsNotExist(err) {
...
func (l *customFS) createNodeData(path, data string) error {
	realPath := l.formatPath(path)
	dataPath := l.getDataPath(realPath)
	return ioutil.WriteFile(dataPath, []byte(data), fileMode)
}

func (l *customFS) CreateTempNode(path string, data string) (err error) {
...
func (l *customFS) CreateSeqNode(path string, data string) (rpath string, err error) {
	nid := time.Now().UnixNano()
	rpath = fmt.Sprintf("%s_%d", path, nid)
	return rpath, l.CreateTempNode(rpath, data)
}

func (l *customFS) GetSeparator() string {
	return string(filepath.Separator)
}

func (l *customFS) Close() error {
	l.tempNodeLock.Lock()
	defer l.tempNodeLock.Unlock()
	if l.done {
		return nil
