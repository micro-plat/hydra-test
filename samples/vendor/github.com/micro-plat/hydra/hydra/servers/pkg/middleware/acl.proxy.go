...
func useProxy(ctx IMiddleContext, cluster *proxy.UpCluster) {
	...
	if req.Header.Get("proxy") == ctx.APPConf().GetServerConf().GetServerID() {
		ctx.Log().Info("服务多次经过当前服务器")
		ctx.Response().Abort(502, fmt.Errorf("服务多次经过当前服务器"))
		return
	}
	//处理重试问题
	var num, max = 0, 10
RETRY:
	...
	//转到上游
	req.Header.Set("proxy", ctx.APPConf().GetServerConf().GetServerID())
