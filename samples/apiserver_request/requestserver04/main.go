...
// apiserver-request-formdata/urlencode请求数据传输demo

//1.2 使用 ./requestserver04 run
//1.3 调用接口：http://localhost:8070/hydratest/apiserver/trace 判定配置是否正确
//1.4 请求的数据：
/*
通过postman构建如下请求：
1.1 请求路由无编码格式接口-Get-urlEncode-gbk-与头编码相同  中文数据无法获取
1.2 请求路由无编码格式接口-Get-urlEncode-gbk-与头编码不同  中文数据无法获取
1.3 请求路由无编码格式接口-Get-urlEncode-gb2312-与头编码相同  中文数据无法获取
1.4 请求路由无编码格式接口-Get-urlEncode-gb2312-与头编码不同  中文数据无法获取
1.5 请求路由无编码格式接口-Get-urlEncode-utf8-与头编码相同  中文数据无法获取
1.6 请求路由无编码格式接口-Get-urlEncode-utf8-与头编码不同  中文数据无法获取
1.7 请求路由无编码格式接口-Post-urlEncode-gbk-与头编码相同  中文数据无法获取
1.8 请求路由无编码格式接口-Post-urlEncode-gbk-与头编码不同  中文数据无法获取
1.9 请求路由无编码格式接口-Post-urlEncode-gb2312-与头编码相同  中文数据无法获取
...
2.3 请求路由有编码格式接口-Get-urlEncode-gb2312-与路由编码相同  中文数据无法获取
2.4 请求路由有编码格式接口-Get-urlEncode-gb2312-与路由编码不同  中文数据无法获取
2.5 请求路由有编码格式接口-Get-urlEncode-utf8-与路由编码相同  中文数据无法获取
2.6 请求路由有编码格式接口-Get-urlEncode-utf8-与路由编码不同  中文数据无法获取
2.7 请求路由有编码格式接口-Post-urlEncode-gbk-与路由编码相同  中文数据无法获取
2.8 请求路由有编码格式接口-Post-urlEncode-gbk-与路由编码不同  中文数据无法获取
2.9 请求路由有编码格式接口-Post-urlEncode-gb2312-与路由编码相同  中文数据无法获取
...
var funcRequest = func(ctx hydra.IContext) (r interface{}) {
	ctx.Log().Info(" apiserver-request-formdata/urlencode/Get请求数据传输demo")
...
var funcRequest1 = func(ctx hydra.IContext) (r interface{}) {
	ctx.Log().Info(" apiserver-request-gbk- formdata/urlencode/Get请求数据传输demo")
	if err := ctx.Request().Check(); err != nil {
		ctx.Log().Errorf("ctx.Request().Check()异常：%s", err)
	}
	param := &xml{}
...
var funcRequest2 = func(ctx hydra.IContext) (r interface{}) {
	ctx.Log().Info("apiserver-request-gb2312- formdata/urlencode/Get请求数据传输demo")
	if err := ctx.Request().Check(); err != nil {
		ctx.Log().Errorf("ctx.Request().Check()异常：%s", err)
	}
	param := &xml{}
...
var funcRequest3 = func(ctx hydra.IContext) (r interface{}) {
	ctx.Log().Info("apiserver-request-utf8- formdata/urlencode/Get请求数据传输demo")
	if err := ctx.Request().Check(); err != nil {
		ctx.Log().Errorf("ctx.Request().Check()异常：%s", err)
	}
	param := &xml{}
