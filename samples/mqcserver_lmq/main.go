...
func init() {
	hydra.Conf.API("8072")
	hydra.Conf.MQC(lmq.MQ).Queue(queue.NewQueue("mqcserver:queue2", "/hydratest/mqcserver/queue2"))
...
var funcMQC1 = func(ctx hydra.IContext) (r interface{}) {
	ctx.Log().Debug("mqcserver-queue-mqc1-11111")
...
var funcMQC2 = func(ctx hydra.IContext) (r interface{}) {
	ctx.Log().Debug("mqcserver-queue-mqc2-22222 ")
	xmap := ctx.Request().GetMap()
	ctx.Log().Info("ctx.Request().GetMap()：", xmap)
	return
}
